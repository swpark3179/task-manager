...
const buildDemoEvents = () => {
  ...
  return [
    ...
    // ---- busy-day sample: 17일 = 일정 3 + 작업 10 ----
    { id: "e18", cat: "company", title: "배포 점검 회의",
      start: D(17, "11:00"), end: D(17, "12:00"), detail: "v2.3 릴리스 체크리스트" },
    { id: "e19", cat: "company", title: "디자인 QA 싱크",
      start: D(17, "15:00"), end: D(17, "15:40"), detail: "통합 토큰 적용 리뷰" },
...
// Demo tasks (할일) — each due on a given day. 17일에 10개를 몰아
// 작업이 많은 날 케이스를 보여준다.
const buildDemoTasks = () => {
  const today = new Date();
  const y = today.getFullYear();
  const m = today.getMonth() + 1;
  const D = (day) => `${y}-${pad2(m)}-${pad2(day)}`;
  const mk = (id, day, title, cat, status) => ({ id, due: D(day), title, cat, status });
  return [
    // 17일 — 10개 (혼합 상태)
    mk("t1",  17, "통합 토큰 머지 후 회귀 테스트", "company", "doing"),
    mk("t2",  17, "릴리스 노트 v2.3 작성",       "company", "todo"),
    mk("t3",  17, "디자인 QA 코멘트 12건 반영",   "company", "doing"),
    mk("t4",  17, "캘린더 모바일 레이아웃 검수",   "company", "todo"),
    mk("t5",  17, "스토리북 스냅샷 업데이트",       "company", "todo"),
    mk("t6",  17, "접근성 대비 점검(상태 4색)",     "company", "done"),
    mk("t7",  17, "PR 리뷰 3건",                  "company", "done"),
    mk("t8",  17, "병원 예약 변경 전화",            "personal", "todo"),
    mk("t9",  17, "장보기 목록 정리",              "personal", "todo"),
    mk("t10", 17, "독서 모임 책 챕터 3 읽기",       "personal", "discarded"),
    // 다른 날 — 분산
    mk("t11", 2,  "주간보고 초안",                 "company", "todo"),
    mk("t12", 8,  "시안 피드백 정리",              "company", "doing"),
    mk("t13", 8,  "회의록 공유",                   "company", "todo"),
    mk("t14", 11, "고객사 자료 준비",              "company", "todo"),
    mk("t15", 11, "세금계산서 발행",               "personal", "done"),
    mk("t16", 20, "가족 저녁 식당 예약",            "personal", "todo"),
    mk("t17", 25, "코드 리뷰 가이드 갱신",          "company", "todo"),
    mk("t18", 25, "연차 신청",                     "personal", "todo"),
  ];
};

// Group tasks by due day → Map<ISO, task[]>
const tasksByDay = (tasks) => {
  ...
  const order = { doing: 0, todo: 1, done: 2, discarded: 3 };
...
// Birthdays / anniversaries (기념일·생일) — anchored to current month.
// Holidays come from holidayMapForYear separately; these add the
// personal "special day" tier (priority 1 alongside holidays).
const buildSpecialDays = () => {
  ...
  const D = (day) => `${y}-${pad2(m)}-${pad2(day)}`;
  return [
    { id: "sp1", date: D(12), title: "어머니 생신", kind: "birthday" },
    { id: "sp2", date: D(17), title: "결혼기념일", kind: "anniv" },
    { id: "sp3", date: D(24), title: "창립기념일", kind: "anniv" },
  ];
};

// Merge holidays + birthdays/anniversaries → Map<ISO, [{title, kind}]>
// kind: 'holiday' | 'birthday' | 'anniv'
const specialsByDay = (year, month) => {
  const map = new Map();
  const push = (iso, item) => { if (!map.has(iso)) map.set(iso, []); map.get(iso).push(item); };
  const hmap = holidayMapForYear(year);
  for (const [iso, title] of Object.entries(hmap)) {
    if (iso.slice(0, 7) === `${year}-${pad2(month)}`) push(iso, { title, kind: "holiday" });
  }
  for (const s of buildSpecialDays()) push(s.date, { title: s.title, kind: s.kind });
  return map;
};
...
window.CalendarData = {
  ...
  holidayMapForYear,
  buildDemoTasks, tasksByDay,
  buildSpecialDays, specialsByDay,
};
